...
class Base:
    ...
    def __init__(self, k, n, basis_options, file_root,
                axes  = ['magnitude','colour','position'],
                lengthscale_m = 1.0, lengthscale_c = 1.0,
                M = None, C = None, Mlim=[-100,100], Clim=[-100,100],
                nside = None, sparse = False, sparse_tol = 1e-4, pivot = False, pivot_tol = 1e-4,
                nest = True, mu = None, sigma = None,
                spherical_basis_directory='./SphericalBasis',
                stan_model_directory='./StanModels',
                output_directory='./StanOutput'):
    ...
    def optimize(self, number_of_iterations = 1000, inits = 2):
        ...
        t1 = time.time()
        _stan_optimum = self.stan_model.optimize(data = self.stan_input, iter = number_of_iterations, output_dir = self.output_directory, inits = inits)
        t2 = time.time()
    ...
    def save_h5(self, runtime, *keys):
        ...
        with h5py.File(os.path.join(self.output_directory, self.optimum_results_file), 'w') as orf:
            ...
            orf['x'].attrs['Mlim'] = self.Mlim
            orf['x'].attrs['Clim'] = self.Clim
            orf['x'].attrs['lengthscale_m'] = self.lengthscale_m
            orf['x'].attrs['lengthscale_c'] = self.lengthscale_c
            orf['x'].attrs['j'] = self.j
            orf['x'].attrs['B'] = self.B
            for key in keys:
    ...
    def _load_spherical_basis(self):
        ...
        with h5py.File(os.path.join(self.spherical_basis_directory, self.spherical_basis_file), 'r') as sbf:
